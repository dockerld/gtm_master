...
const CANON_ORGS_CFG = {
  ...
  CANON_HEADERS: [
    ...
    // Trial extension tracking (covers in-app promo, Stripe promo, manual Stripe extension)
    'trial_extended',          // TRUE if trial_ends_at > org_created_at + 14 days
    'trial_extension_source',  // in_app_promo | stripe_promo | stripe_manual | combo
  ...
  lockWrap('build_canon_orgs', () => {
    ...
    try {
      ...
      posthogOrgSubsRows.rows.forEach(r => {
        ...
        // Track earliest trial_ends_at from PostHog
        const phTrialEnd = orgBuildParseDate_(orgBuildRawGet_(posthogOrgSubsRows, r, ['trial_ends_at']))
        if (phTrialEnd && (!agg.earliest_trial_ends_at || phTrialEnd < agg.earliest_trial_ends_at)) {
          agg.earliest_trial_ends_at = phTrialEnd
        }
      })

      // ---- 5) Build app promo redemption aggregates (PostHog promo_redemptions) ----
      // Include ALL redemption locations (in_app, stripe, checkout, etc.)
      // so expired/cancelled orgs retain their promo tag
      ...
      orgRows.rows.forEach(o => {
        ...
        // trial_ends_at: scan ALL linked subs for earliest trial_end,
        // fallback to PostHog, then org_created_at + 14 days
        const trialEndsAt = (() => {
          let earliest = null
          for (const sid of linkedSubIds) {
            const s = stripeBySubId.get(sid)
            if (!s) continue
            const te = orgBuildParseDate_(s.trial_end)
            if (te && (!earliest || te < earliest)) earliest = te
          }
          // Fallback to PostHog trial_ends_at
          if (!earliest && posthogAgg && posthogAgg.earliest_trial_ends_at) {
            earliest = posthogAgg.earliest_trial_ends_at
          }
          // Last resort: org_created_at + 14 days
          if (!earliest) {
            const created = orgBuildParseDate_(orgCreatedAt)
            if (created) {
              const defaultEnd = new Date(created.getTime())
        ...
        outRows.push([
          ...
          // Trial extension detection
          // Excludes bulk Stripe migration on 2026-02-28 (not a real extension)
          ...(() => {
            const created = orgBuildParseDate_(orgCreatedAt)
            if (!created || !trialEndsAt || !(trialEndsAt instanceof Date)) return [false, '']
            const defaultEnd = new Date(created.getTime())
            defaultEnd.setDate(defaultEnd.getDate() + 14)
            const isExtended = trialEndsAt.getTime() > defaultEnd.getTime() + (12 * 60 * 60 * 1000)
            if (!isExtended) return [false, '']
            const sources = []
            if (appPromoCodesText) sources.push('in_app_promo')
            if (stripePromoCodesText) sources.push('stripe_promo')
            if (sources.length === 0) {
              // Check if trial_ends_at is 2026-02-28 — bulk migration, not a real manual extension
              const teKey = trialEndsAt.getFullYear() + '-' +
                String(trialEndsAt.getMonth() + 1).padStart(2, '0') + '-' +
                String(trialEndsAt.getDate()).padStart(2, '0')
              if (teKey === '2026-02-28') return [false, '']
              sources.push('stripe_manual')
            }
            return [true, sources.join(', ')]
          })(),
