...
internal_subs AS (SELECT DISTINCT org_id FROM postgres.org_subscriptions WHERE stripe_subscription_id IN (SELECT subscription_id FROM override_googlesheets_manual_stripe_changes WHERE lower(exclude_reason)='internal')),
-- Internal/test orgs hidden from The Ring: founder email domain/explicit list, OR any sub flagged 'internal' in the override sheet.
-- To hide more, add emails/domains to the list below.
internal_orgs AS (SELECT o.id AS org_id FROM (SELECT id FROM postgres.orgs LIMIT 1 BY id) o LEFT JOIN founder f ON f.org_id=o.id WHERE lower(coalesce(f.email,'')) LIKE '%@pingassistant.com' OR lower(coalesce(f.email,'')) LIKE '%@pingassistant.dev' OR lower(coalesce(f.email,'')) IN ('dockerld@gmail.com','testpingassistant@gmail.com','gptfam69@gmail.com','chad@bookends.app','stormmgarnett@gmail.com') OR o.id IN (SELECT org_id FROM internal_subs)),
