...
-- MANAGED orgs: keyed on the sheet's org_id. ARR = 'amount'; seats = full_seats+lite_seats. Injected at per_org (no Stripe sub/invoice needed).
...
org_arr AS (SELECT osb.org_id, round(sum(ci.arr),2) AS arr_actual FROM postgres.org_subscriptions osb JOIN ci ON ci.sid=osb.stripe_subscription_id WHERE osb.stripe_subscription_id IN (SELECT subscription_id FROM paid) AND osb.org_id NOT IN (SELECT org_id FROM sheet_excl) AND osb.status!='canceled' AND ci.arr>0 GROUP BY osb.org_id),
