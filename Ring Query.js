...
managed_ovr AS (SELECT subscription_id, toFloat64OrNull(replaceRegexpAll(coalesce(amount,''),'[^0-9.]','')) AS ovr_arr, coalesce(quantity,0) AS ovr_seats FROM override_googlesheets_manual_stripe_changes WHERE lower(exclude_reason)='managed' AND subscription_id IS NOT NULL),
per_org AS (SELECT o.id AS org_id, if(mo.subscription_id IS NOT NULL, mo.ovr_seats, coalesce(picked.full_seat_count,0)+coalesce(picked.lite_seat_count,0)) AS seats, (pm.customer_id IS NOT NULL) AS has_pm, (fp_org.org_id IS NOT NULL OR (mo.subscription_id IS NOT NULL AND mo.ovr_arr>0)) AS has_fp, (picked.app_status IN ('active','trialing')) AS is_live, if(mo.subscription_id IS NOT NULL AND mo.ovr_arr IS NOT NULL, mo.ovr_arr, coalesce(org_arr.arr_actual,0)) AS arr_actual, round(coalesce(sx.period_amt,0)*if(sx.intv='month',12,1),2) AS arr_potential FROM (SELECT id FROM postgres.orgs LIMIT 1 BY id) o LEFT JOIN picked ON picked.org_id=o.id LEFT JOIN sx ON sx.sub_id=picked.stripe_subscription_id LEFT JOIN fp_org ON fp_org.org_id=o.id LEFT JOIN org_arr ON org_arr.org_id=o.id LEFT JOIN pm ON pm.customer_id=picked.stripe_customer_id LEFT JOIN managed_ovr mo ON mo.subscription_id=picked.stripe_subscription_id),
