...
org_arr AS (SELECT osb.org_id, round(sum(if(m.sid IS NOT NULL, m.amt, sr.arr)),2) AS arr_actual FROM postgres.org_subscriptions osb JOIN sub_rev sr ON sr.sid=osb.stripe_subscription_id LEFT JOIN managed m ON m.sid=osb.stripe_subscription_id WHERE osb.stripe_subscription_id IN (SELECT subscription_id FROM paid) AND osb.stripe_subscription_id NOT IN (SELECT subscription_id FROM sheet_excl) AND osb.status!='canceled' AND if(m.sid IS NOT NULL, m.amt, sr.arr)>0 GROUP BY osb.org_id),
...
paid AS (SELECT subscription_id FROM stripe.invoice WHERE status='paid' AND toFloat(total)>0 AND subscription_id IS NOT NULL GROUP BY subscription_id),
sheet_excl AS (SELECT subscription_id FROM override_googlesheets_manual_stripe_changes WHERE lower(exclude_reason) != 'managed'),
managed AS (SELECT subscription_id, toFloat(nullIf(replaceAll(replaceAll(toString(amount),'$',''),',',''),'')) AS amt FROM override_googlesheets_manual_stripe_changes WHERE lower(exclude_reason)='managed'),
org_arr AS (SELECT osb.org_id, round(sum(if(m.subscription_id IS NOT NULL, m.amt, sr.arr)),2) AS arr_actual, max(m.subscription_id IS NOT NULL) AS is_managed FROM postgres.org_subscriptions osb JOIN sub_rev sr ON sr.sid=osb.stripe_subscription_id LEFT JOIN managed m ON m.subscription_id=osb.stripe_subscription_id WHERE osb.stripe_subscription_id IN (SELECT subscription_id FROM paid) AND osb.stripe_subscription_id NOT IN (SELECT subscription_id FROM sheet_excl) AND osb.status!='canceled' AND if(m.subscription_id IS NOT NULL, m.amt, sr.arr)>0 GROUP BY osb.org_id),
picked AS (SELECT org_id, stripe_subscription_id, status AS app_status, trial_ends_at, full_seat_count, lite_seat_count FROM postgres.org_subscriptions ORDER BY (stripe_subscription_id IN (SELECT subscription_id FROM paid) AND status!='canceled') DESC, (status='active') DESC, (status='trialing' AND trial_ends_at>now()) DESC, created_at DESC LIMIT 1 BY org_id),
excl_emails AS (SELECT DISTINCT lower(customer_email) AS email FROM override_googlesheets_manual_stripe_changes WHERE lower(exclude_reason) IN ('internal','duplicate','fake') AND coalesce(customer_email,'')!=''),
founder_x AS (SELECT uo.org_id AS org_id, argMin(lower(u.email), uo.created_at) AS email FROM postgres.users_orgs uo JOIN postgres.users u ON u.id=uo.user_id GROUP BY uo.org_id),
excluded_orgs AS (SELECT DISTINCT os.org_id AS org_id FROM postgres.org_subscriptions os JOIN (SELECT id, lower(email) AS email FROM stripe.customer LIMIT 1 BY id) c ON c.id=os.stripe_customer_id WHERE c.email IN (SELECT email FROM excl_emails) UNION DISTINCT SELECT org_id FROM founder_x WHERE email IN (SELECT email FROM excl_emails)),
paid_orgs AS (SELECT o.id AS org_id, org_arr.arr_actual AS arr, org_arr.is_managed AS is_managed, coalesce(picked.full_seat_count,0)+coalesce(picked.lite_seat_count,0) AS seats FROM (SELECT id FROM postgres.orgs LIMIT 1 BY id) o JOIN org_arr ON org_arr.org_id=o.id LEFT JOIN picked ON picked.org_id=o.id WHERE org_arr.arr_actual>=0.01 AND o.id NOT IN (SELECT org_id FROM excluded_orgs))
